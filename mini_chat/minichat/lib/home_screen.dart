...
class _HomeState extends State<Home> {
  ...
  void _updateTokenInDb(String token) async {
    await firebaseFirestore.collection("users").doc(firebaseAuthInstance.currentUser!.uid).update({'fcm': token});
  }
              ...
              FutureBuilder(
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) {
                      return CircleAvatar(backgroundColor: Colors.white, radius: 40, foregroundImage: NetworkImage(snapshot.data!));
                    }
